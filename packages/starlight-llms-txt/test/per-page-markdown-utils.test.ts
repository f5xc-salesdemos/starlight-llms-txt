...
describe("slugToPath", () => {
	it("appends .html for the append strategy", () => {
		expect(slugToPath("getting-started", "append")).toBe(
	...
	it("handles nested slugs", () => {
		expect(slugToPath("demo/phase-1-build", "append")).toBe(
			"demo/phase-1-build.html",
		);
		expect(slugToPath("demo/phase-1-build", "replace")).toBe(
			"demo/phase-1-build",
		);
	});
});
